...
// Helpers for the throwaway Gridwell homes the e2e suite creates (seedHome in
...
export function pluginUUIDs(home: string): string[] {
  ...
  for (const line of src.split('\n')) {
    // A top-level `id: <id>` or a YAML list entry `    - id: <id>`. Without the
    // optional dash the per-test tmux kill matches nothing and servers leak.
    // Both minted id shapes are valid forever, 32-hex and the 7-char base36
    // short form, and matching only one leaks a tmux server for every id of the
    // other.
...
// shell tiles leaves those sessions to interfere with the next test. It is a
...
// whose server is gone. The per-test teardown already cleans up, but a teardown
// that exceeds the test timeout gets the whole worker SIGKILLed and never runs,
// and those leaks accumulate. Sweeping at the start of each run survives any
// kind of kill. Only e2e-created homes are touched, by the gridwell-e2e-
// mkdtemp prefix, never the user's real ~/.gridwell.
...
// gone it is indistinguishable from the user's real desktop app. The per-test
// teardown owns killing its own servers.
