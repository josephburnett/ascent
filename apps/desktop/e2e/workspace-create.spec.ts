...
// drag-creating it persists a kind="pane" tile server-side, and a foreign
// layout write reaches this client's preview-relevant state. A pane layout is
// framing-class, so the version stays put and the event echo arrives at the
// same version with only the blob field differing. For the preview signature to
// move, a same-version event must still apply: the interlock drops only
// strictly older events. The stale content-bytes half of that echo,
// cache.Apply's blob-change drop, is pinned by unit test in client/cache.
...
test('workspace primitive: drag-create persists a pane tile and its preview tracks the layout blob', async ({ gw, window }) => {
  ...
  // dragCreate resolves the swatch by its templateKindName, so a missing
  // palette entry fails right here.
  ...
  // the client cache refresh is postTileMutate's background fetchGrid and
  // waitIdle can sample the gap between the RPC completing and that goroutine
  // marking itself in flight.
  await expect
