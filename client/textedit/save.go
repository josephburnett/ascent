...
// writer's event without this client seeing the new bytes. The row version
// stands in only when the content entry is gone and that row owns the bytes;
// a link row tracks its placement, so its claim is 0. rowVersion is the
// snapshot the flush decided from, never re-read at send time.
