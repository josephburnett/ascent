...
func TestParseServeFlagsDefaults(t *testing.T) {
	...
	// Unset is what lets resolveBind fall through to the config default.
...
// The listen-address precedence: --bind > server.yaml web.bind (present) >
// --bind-default > the built-in default. Present is config.Load's BindSet, so
// a config bind equal to the built-in default still pins the address.
...
// The sidecar boot contract lines.ts parses: the web address leads, auth= is
// the derived token, and federation= is last because a socket path may
// contain spaces.
...
// A missing config is a fresh home: id minted, file written, store created.
...
// A plugin listed without an id gets one minted and written back.
...
// The guard against a silently new database: an edited id must be a hard
// error, not a fresh empty store beside the real one.
