...
func TestAltFromSource(t *testing.T) {
	cases := []struct {
		...
		{"H2 heading", "## Sub", "Sub"},
		// goldmark joins the two physical lines into one paragraph, so the alt
		// summarizes the whole first paragraph.
		...
		// The alt is one line by contract, so the block's newlines collapse.
